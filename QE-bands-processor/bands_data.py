...
class PWscfBandsOutputReader(FileReader):
    def __init__(self, read_path):
        super().__init__(
            reader_states={
                "state": PWscfBandsOutputState(self)
            }, 
...
class EnergyBand:
    def __init__(self, eigv, k_points_x, spins):
        self.eigv = np.array(eigv)
        self.k_points_x = np.array(k_points_x)
        self.spins = np.array(spins)
        if self.eigv.ndim != 1:
            raise ValueError(f"eigv must be one-dimensional, but has {self.eigv.ndim} dimensions instead.")
        if self.k_points_x.ndim != 1:
            raise ValueError(f"k_points_x must be one-dimensional, but has {self.k_points_x.ndim} dimensions instead.")
        if self.spins.ndim != 1:
            raise ValueError(f"spins must be one-dimensional, but has {self.spins.ndim} dimensions instead.")
        if not (np.array([self.eigv.size, self.k_points_x.size, self.spins.size]) == self.eigv.size).all():
            raise ValueError(f"Input shape of eigv, k_points_x, spins are not same: {self.eigv.shape}, {self.k_points_x.shape}, {self.spins.shape}")

        self.k_count = self.eigv.size

    @property
    def max_eigv(self):
        return self.eigv[self.eigv == np.max(self.eigv)]
    ...
    @property
    def min_eigv(self):
        return self.eigv[self.eigv == np.min(self.eigv)]
...
class BandsDataState(FileReaderState):
    ...
    def execute(self):
        tokens = list(filter(None, self._file_reader.current_str.strip().split(" ")))
        if len(tokens) == 0:
            self._bands.append(EnergyBand(self._current_eigv, self._current_k_points, []))
        else:
            self._current_eigv.append(float(tokens[0]))
            self._current_k_points.append(float(tokens[1]))

    def exit(self):
        self._file_reader.band_structure = BandStructure(self._bands, self._file_reader.n_electrons)
